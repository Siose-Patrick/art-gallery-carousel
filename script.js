window.addEventListener("load", () => {
    ...
    function compressImage(img, quality = 0.5) {
        return new Promise((resolve) => {
            ...
            // Set canvas to same dimensions as image
            ...
            // Convert to JPG with specified quality (0.5 = 50% quality)
            const compressedDataUrl = canvas.toDataURL('image/jpeg', quality);
            
            const compressedImg = new Image();
            compressedImg.onload = function() {
                // Log compression results
                const originalSize = img.src.length * 0.75; // Approx base64 size
    ...
    async function loadImage() {
        for (let i = 0; i < 7; i++) {
            ...
            // REMOVE crossOrigin attribute for local files
            // img.crossOrigin = "anonymous"; // This causes CORS issues with file://
            
            img.onload = async function() {
                try {
                    // Compress the JPG image to 50% quality
                    ...
                    images.push(img); // Fallback to original
                }
            ...
            // Since your files are JPG, update the extension
            img.src = `./public/assets/images/img${i}.jpg`;
        }
    }

    // The rest of your initializeScene function remains EXACTLY THE SAME
